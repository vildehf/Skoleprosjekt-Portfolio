...
// ==========================
// Toggle active button in form-card
// ==========================
document.querySelectorAll(".form-card").forEach(group => {
  ...
  buttons.forEach(button => {
    button.addEventListener("click", () => {
      // Hvis allerede aktiv, fjern aktiv
...
function renderPets() {
  ...
  pets.forEach(pet => {
    ...
    petDiv.classList.add("card", "active"); // legger til .card og .active
    petDiv.innerHTML = `
      <h4>${pet.name}</h4>
